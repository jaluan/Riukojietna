function out = stone2000RcTT(h,Rc)
...
if length(h) > 1;
    error('stone2000RcTT.m -- not vectorized in h');
end;

if max(Rc) > 23;
    error('Rc greater than 23 in stone2000RcTT.m');
end;
...
% ilats_r = d2r(ilats_d.*pi./180; % convert to radians
...
iRcs = (dd(1)*cos(d2r(ilats_d)) + ...
   dd(2)*(cos(d2r(ilats_d))).^2 + ...
   dd(3)*(cos(d2r(ilats_d))).^3 + ...
   dd(4)*(cos(d2r(ilats_d))).^4 + ...
   dd(5)*(cos(d2r(ilats_d))).^5 + ...
   dd(6)*(cos(d2r(ilats_d))).^6);
...
add_sf = exp( log(sf(1)) + fits(1).*( log(23:-1:16) - log(iRcs(1)) ) ) ;
sf = [add_sf sf];
iRcs = [23 22 21 20 19 18 17 16 iRcs];
